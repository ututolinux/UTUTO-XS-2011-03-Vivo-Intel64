...
function configurarLilo() {
	...
	for kernel in $KERNELS; do
		...
		echo "image=/boot/kernel-${!version}" >> $LILOCONF
		echo "       label=${!etiqueta}" >> $LILOCONF
		echo "       root=/dev/ram0" >> $LILOCONF
		echo "       initrd=/boot/initramfs-${!version}" >> $LILOCONF
		echo "       append=\"${!parametros}\"" >> $LILOCONF
		echo "       addappend=\"real_root=$ROOTDEV real_resume=swap:$DEVICESWAP resume=swap:$DEVICESWAP resume2=swap:$DEVICESWAP\"" >> $LILOCONF
		echo "" >> $LILOCONF
	done
	BUSGRUB="0"
	HDDGRUB="0"
	...
	sed -i "s/DEVICEGRUB1/$DEVICEGRUB/" $GRUBCONF
	sed -i "s/DEVICEGRUB1/$DEVICEGRUB/" $GRUBCONF

	sed -i "s/DEVICEGRUBR/$DEVICESWAPG/" $GRUBCONF
	sed -i "s/DEVICEGRUBR/$DEVICESWAPG/" $GRUBCONF
	sed -i "s/DEVICEGRUBR/$DEVICESWAPG/" $GRUBCONF
	sed -i "s/DEVICEGRUBR/$DEVICESWAPG/" $GRUBCONF
